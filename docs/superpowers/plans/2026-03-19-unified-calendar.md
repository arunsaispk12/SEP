...
**Goal:** Replace `ScheduleCalendar` and `CaseCalendarView` with a single `UnifiedCalendar` component that shows cases as calendar events, allowing admins to create/edit/delete cases directly from the calendar.

**Architecture:** Cases are the sole source of truth — they already have `scheduled_start`, `scheduled_end`, and `assigned_engineer_id`. A new `case_type` column distinguishes client work from internal blocks. The unified calendar uses the existing gcal-layout shell (sidebar + topbar + FullCalendar), reads only from `cases` via EngineerContext, and replaces both the Schedule tab component and the CaseCalendarView toggle inside CaseManager.

**Tech Stack:** React 18, FullCalendar v6 (`@fullcalendar/react`, timegrid, daygrid, interaction, list), Supabase, existing gcal-layout CSS.
...
|------|--------|---------------|
| `src/components/UnifiedCalendar.jsx` | **Create** | All-in-one calendar: layout, events, form, popup |
| `src/context/EngineerContext.js` | **Modify** | Add `deleteCase` function and expose in context value |
| `src/App.js` | **Modify** | Swap `ScheduleCalendar` import/usage for `UnifiedCalendar` |
| `src/components/CaseManager.js` | **Modify** | Remove calendar toggle, sync button, CaseCalendarView |
| `src/components/ScheduleCalendar.js` | **Delete** | Replaced by UnifiedCalendar |
| `src/components/CaseCalendarView.jsx` | **Delete** | Replaced by UnifiedCalendar |
| `src/services/scheduleCaseSync.js` | **Delete** | No longer needed |

---

## Task 1: Add deleteCase to EngineerContext

**Files:**
- Modify: `src/context/EngineerContext.js`

> `supabaseService.deleteCase(id)` already exists (line 164 of supabaseService.js) but is not exposed through EngineerContext. Add it now before UnifiedCalendar needs it.

- [ ] **Step 1: Add deleteCase function inside EngineerContext**

Find the `updateCase` useCallback (around line 441). After it, add:
...
- [ ] **Step 2: Add DELETE_CASE to the reducer**

Find the reducer function (search for `case 'ADD_CASE'`). Add a case for DELETE_CASE:
...
- [ ] **Step 3: Expose deleteCase in the context value**

Find the value object returned by the context provider (around line 670, the big object with `addCase`, `updateCase`, etc.). Add `deleteCase` to it:
...
git commit -m "feat: expose deleteCase in EngineerContext"
```

---

## Task 2: Add case_type column via Supabase migration

**Files:**
- Supabase database (via MCP tool)

> All existing cases default to `'client_work'`. The column enables `'internal'` type for non-client blocks (training, travel).

- [ ] **Step 1: Apply the migration using the Supabase MCP tool**

Use `mcp__supabase__apply_migration` with:

```sql
ALTER TABLE cases
ADD COLUMN IF NOT EXISTS case_type text NOT NULL DEFAULT 'client_work'
CHECK (case_type IN ('client_work', 'internal'));
```

Migration name: `add_case_type_to_cases`
...
Expected: one row showing `case_type`, `text`, `client_work`

- [ ] **Step 3: Commit**

```bash
git add supabase/migrations/
git commit -m "feat: add case_type column to cases table"
```

---

## Task 3: Create UnifiedCalendar.jsx

**Files:**
- Create: `src/components/UnifiedCalendar.jsx`

> This is the main deliverable. It replaces both ScheduleCalendar.js and CaseCalendarView.jsx. Study those files before writing — the patterns (gcal-layout, MiniCalendar, FullCalendar setup, event styling) are all reused here.
>
> Key facts:
> - Cases table has: `id`, `title`, `client_name` (text), `status`, `priority`, `assigned_engineer_id`, `scheduled_start`, `scheduled_end`, `location_id`, `description`, `case_type`
> - EngineerContext maps cases with runtime `location` (string name) and `assignedEngineer` (string name) added at load time
> - `locationObjects` = `[{ id, name }]` — needed to convert location name → location_id for form submit
> - `STATUS_COLORS` from `./dashboard/dashboardUtils` — shape: `{ open: { border, bar }, assigned: { border, bar }, ... }`
> - `ENGINEER_COLORS` — 8 hex colors defined locally (same as in ScheduleCalendar)
> - LocationCombobox is defined locally in ScheduleCalendar — redefine it locally here too
> - `profile.role` from useAuth — `'engineer'` role has restricted actions
> - `user.id` from useAuth — for identifying own cases

- [ ] **Step 1: Create the file with the complete implementation**

Create `src/components/UnifiedCalendar.jsx` with the following complete code:
...
```bash
ls src/components/UnifiedCalendar.jsx
```
...
Expected: `Compiled successfully.` (may show warnings about unused variables — that's fine).

- [ ] **Step 4: Commit**

```bash
git add src/components/UnifiedCalendar.jsx
git commit -m "feat: create UnifiedCalendar component"
```

---

## Task 4: Update App.js — swap ScheduleCalendar for UnifiedCalendar
...
```bash
git add src/App.js
git commit -m "feat: mount UnifiedCalendar in app routing"
```

---

## Task 5: Clean up CaseManager.js

**Files:**
- Modify: `src/components/CaseManager.js`

> Remove three things: (1) the calendar toggle (viewMode state + toggle buttons + conditional render), (2) the Sync with Schedules button and handler, (3) the CaseCalendarView import.
>
> Be careful: `viewMode` state is ONLY used for the calendar toggle. The stats grid wrapping `{viewMode === 'list' && ...}` also needs to be cleaned up — just render the stats grid unconditionally.

- [ ] **Step 1: Remove CaseCalendarView and scheduleCaseSyncService imports**
...
- [ ] **Step 2: Remove viewMode state and isSyncing state**
...
- [ ] **Step 3: Remove handleSyncWithSchedules function**

Find and delete the entire `handleSyncWithSchedules` function (it references `scheduleCaseSyncService`).

- [ ] **Step 4: Remove the Sync button from the header**

In the header actions div, remove:
```jsx
<button
...
Also remove the `RotateCcw` import from lucide-react if it's no longer used elsewhere.

- [ ] **Step 5: Remove the view toggle buttons**

Find and remove the entire view mode toggle section:
```jsx
{/* View mode toggle */}
...
- [ ] **Step 6: Remove the CaseCalendarView conditional render**

Find the block that conditionally renders CaseCalendarView (the `viewMode === 'calendar'` branch) and remove it, keeping only the list view content. The stats grid wrapper `{viewMode === 'list' && <div className="stats-grid">` should become just `<div className="stats-grid">` (render unconditionally).
...
git commit -m "refactor: remove calendar toggle and sync button from CaseManager"
```
...
**Files:**
- Delete: `src/components/ScheduleCalendar.js`
- Delete: `src/components/CaseCalendarView.jsx`
- Delete: `src/services/scheduleCaseSync.js`

- [ ] **Step 1: Delete the three files**

```bash
rm src/components/ScheduleCalendar.js
rm src/components/CaseCalendarView.jsx
rm src/services/scheduleCaseSync.js
```

- [ ] **Step 2: Verify no remaining references**

```bash
grep -r "ScheduleCalendar\|CaseCalendarView\|scheduleCaseSync" src/ --include="*.js" --include="*.jsx"
```
...
```bash
git add -A
git commit -m "chore: delete ScheduleCalendar, CaseCalendarView, scheduleCaseSyncService"
```
...
After all tasks complete, open the app and verify:

- [ ] Calendar tab shows UnifiedCalendar with gcal-layout (sidebar + main)
- [ ] Week view shows horizontal day columns scrolled to 8 AM
- [ ] Month view shows date grid
- [ ] Engineer color toggles work in sidebar
- [ ] Status filter pills work
- [ ] Unscheduled section shows cases without dates, expands on click
- [ ] Clicking empty slot opens New Case form (admin only)
- [ ] `+ New Case` button in topbar opens form
- [ ] Case type toggle (Client Work / Internal) shows/hides client field
- [ ] Submitting form creates a case and it appears on the calendar
- [ ] Clicking an event opens the popup with case detail
- [ ] Admin/manager: Edit + Delete buttons visible in popup
- [ ] Engineer: only status update buttons on own cases
- [ ] Cases tab no longer has calendar toggle
- [ ] Cases tab no longer has "Sync with Schedules" button
- [ ] No console errors
